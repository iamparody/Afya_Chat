...
- [x] UTI card's 3 compound-term `argues_against` warnings — **fixed 2026-09-18, and they were not cosmetic.** Deferring them in the enrichment pass was the wrong call. Dumping `rag.run()` output for Case 3 showed the model emitting the UTI card's `argues_against` terms verbatim into `missing_information` — `"negative nitrite and negative leukocyte esterase on urine dipstick"`, `"normal urinalysis and negative culture"`, `"male sex"` — and then continuing the pattern to generate `"no dysuria"` and `"no urinary frequency"`. Those terms are phrased as *negated findings*, and the model reproduces the negation style. That is precisely the failure mode Case 3's prohibited strings exist to catch. Replaced with atomic non-negated canonical terms (`negative urine dipstick`, `vaginal discharge`, `normal urinalysis`, `negative urine culture`), matching the existing pattern in the Argues Against Findings table (`normal chest X-ray`, `normal HbA1c`). corpus_version 1.7 → 1.9.
  - **Authoring rule this establishes:** `graph.argues_against` terms must be atomic and must not be phrased as negations of a symptom the patient might simply not have been asked about. Negated graph terms leak into `missing_information` and cause the system to assert absent findings.
- [x] Acute bacterial prostatitis — committed 2026-09-18 (MOH Vol 2 §15.4.1 + EAU 2026). **ICD-11 finding:** there is no acute prostatitis entity in ICD-11. The GA91 block codes chronic prostatitis (`GA91.0`) and abscess of prostate (`GA91.1`) specifically, but the acute bacterial form maps only to `GA91.Y` "Other specified inflammatory and other diseases of prostate" — confirmed by enumerating all 9 children of GA91 via the WHO API. ICD-10 is more precise here (`N41.0 Acute prostatitis`). Card uses `icd_search_term` to pin the override; the imprecision is in the classification, not the diagnosis
...
- 2026-09-18: 7/8 GATE PASS after Acute bacterial prostatitis (27 conditions/243 chunks). MOH Vol 2 §15.4.1 + EAU 2026. Coverage 7/7 (Case 13 added). Full suite 311/311 including integration. Neo4j: 5 GU pairwise edges (2 mandatory safety).
  - **Case 3 (UTI vs AGE) is genuinely unstable and should not be read as a signal on its own.** Across eight runs this session on near-identical corpora its leading candidate has been Acute pyelonephritis, Acute gastroenteritis, Urinary tract infection and Typhoid fever. The case is deliberately an incomplete presentation designed to test co-equal candidates, and it destabilises further as more urinary conditions enter the corpus. Regression scores this session: 6, 7, 6, 7, 7, 7, 5, 6, 7.
  - **Generalisable finding: each new card in a domain weakens its neighbours' retrieval rank.** Adding pyelonephritis took semantic share from UTI; adding prostatitis reshuffled it again. Re-run the gate after every card rather than at the end of a domain, and expect a neighbour regression rather than being surprised by one.
  - **Authoring error worth not repeating:** the pyelonephritis `typical_presentation` originally opened "A young non-pregnant woman presents with two to three days of…", which is source-faithful but made the chunk a magnet for Case 3 ("26F, 2 days fever…") regardless of whether loin pain was present. Rewritten to lead with the discriminating findings (loin pain, costovertebral angle tenderness) and treat the demographic as the management-pathway detail Vol 2 actually uses it for. **Lead `typical_presentation` with what makes the diagnosis, not with who gets it.**
  - **Two coverage checks were mis-specified by the author and corrected, not loosened to pass.** Case 12 expected `culture/sensitivity/creatinine` as missing information; the model asked for *pregnancy status*, which is a Vol 2 criterion converting pyelonephritis to complicated infection requiring referral — better reasoning than the check anticipated. Case 13 expected `culture/sensitivity/retention`; the model asked for *loin pain / costovertebral angle tenderness*, the discriminators against pyelonephritis. Both check lists broadened to accept the clinically stronger answer.
  - **A prohibited-string safety check was removed as untestable.** Case 13 originally prohibited `"prostate massage"`, since Vol 2 warns it can induce bacteraemia. But the card itself carries that instruction, so a correct output warning *against* the procedure contains the same substring as an incorrect one recommending it. Substring matching cannot express negation. Moved to a manual check.
