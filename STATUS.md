...
- 2026-09-18: 7/8 GATE PASS after Acute pyelonephritis (26 conditions/234 chunks) — **with a caveat, read before interpreting a future 6/8**. MOH Vol 2 §15.2.1 + EAU 2026 (two-source pattern). Coverage 6/6 (Case 12 Acute pyelonephritis added — leads correctly, 4/4 auto checks). Full suite 303/303 including integration. Gate measured on the identical corpus state as committed.
  ...
  - **Residual: Case 3 intermittently trips the prohibited substring `'no dysuria'`** (2 of 3 post-fix runs; absent on a direct fourth invocation). The leading candidate is correct; the model writes the phrase while reasoning acceptably. Not cleanly attributable — plausibly the new card splits the urinary semantic space and shifts Case 3's lead from UTI to AGE, after which the model argues against UTI by naming the absent symptom. Unconfirmed.
...
- [ ] Urinary tract infection — existing card, enrichment pass only (complicated-UTI referral trigger, male-UTI rule, DIFFERENTIATED_FROM edges)
